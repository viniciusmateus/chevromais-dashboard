...
export default function Impugnar() {
	...
	// --- FUNÇÃO DE FORMATAÇÃO DO ÓRGÃO ---
	const getFinalBuyer = () => {
		if (buyer === "PREFEITURA MUNICIPAL DE") {
			return `${buyer} ${cityUF}`.trim();
		}
		if (buyer === "COM LOCALIDADE") {
			// Se digitou algo, formata "TEXTO - CIDADE". Se não, manda só a cidade para não quebrar.
	...
	const handleGenerate = async () => {
		...
		// Aplica a regra de negócio do órgão antes de enviar para a API
	...
	const handleCopy = (type, file) => {
		let text = "";
		const finalBuyer = getFinalBuyer(); // Resgata o nome formatado para a cópia
		...
		<div className="p-6 flex flex-col gap-6 max-w-6xl mx-auto bg-zinc-950 text-zinc-200 min-h-screen">
			...
			<div className="bg-zinc-900/60 p-5 rounded-xl border border-zinc-800 shadow-xl flex flex-col gap-6">
				...
				<div className="border-t border-zinc-800/80 pt-5">
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
						...
						<div>
							...
							<select
								...
								className={`${inputStyle} h-[42px] cursor-pointer`}
							>
					...
					{/* Input extra liberado para ambas opções */}
