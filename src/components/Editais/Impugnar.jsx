...
export default function Impugnar() {
    ...
    // --- CHAMADA EXCLUSIVA PARA A NOVA API NO CLOUD RUN ---
    const handleGenerate = async () => {
        ...
        const tasks = activeObjections.map(async (objectionKey) => {
            ...
            const payload = {
                ...
                municipioUF: cityUF,
                orgao: buyer === "OUTRO" ? otherBuyer : buyer,
                objection: objectionKey,
                prazoEntrega: {
                    prazo: deliveryStipulate,
                    unidade: deliveryUnit,
                },
                amostra: {
                    objeto: sampleObject,
                    clausulas: sampleClause,
                },
                ence: {
                    especificacoes: selectedEnceSpecs,
                    classificacoes: selectedEnceGrades,
                },
                servicoMontagem: {
                    tiposServico: serviceType,
                    objetoServico: serviceObject,
                },
                restricaoRegional: {
                    clausulas: restrictionClause,
                },
            };

            try {
                // Aqui batemos DIRETAMENTE na URL da API do Cloud Run, esquecendo o antigo
                    ...
                    if (contentType && contentType.includes("application/pdf")) {
                        ...
                        setItemStatuses((prev) => ({
                            ...
                            [objectionKey]: {
                                ...
                                file: {
                                    pdfUrl,
                                    pdfFilename: `Impugnacao_${objectionKey}_${disputeNumber || "edital"}.pdf`,
                                    objectionKey,
                                    objectionName: labelName,
                                },
                                error: null,
                            },
                        }));
                        ...
                        setItemStatuses((prev) => ({
                            ...
                            [objectionKey]: {
                                status: "success",
                                file: {
                                    pdfUrl: result.file?.pdfBase64 ? `data:application/pdf;base64,${result.file.pdfBase64}` : result.file?.url,
                                    pdfFilename: result.file?.pdfFilename || `Impugnacao_${objectionKey}.pdf`,
                                    objectionKey,
                                    objectionName: result.file?.objectionName || objectionKey,
                                },
                                error: null,
                            },
                        }));
                ...
                <div className="border-t border-zinc-800/80 pt-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" placeholder="DD/MM/AAAA" maxLength={10} className={inputStyle} value={disputeDate} onChange={handleDateChange} />
                        </div>
                        <div>
                    ...
                    <div className="flex flex-col gap-2.5">
                        <div className="flex justify-between items-end">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-xs font-mono font-bold text-lime-400 bg-lime-400/10 px-2.5 py-1 rounded-md border border-lime-400/20">{completedItems} / {totalItems} concluidos</span>
                                ...
                                <div key={key} className={`flex flex-col md:flex-row md:items-center justify-between p-3.5 rounded-lg border transition-all ${item.status === "success" ? "bg-zinc-950/80 border-lime-400/30 shadow-sm shadow-lime-400/5" : item.status === "processing" ? "bg-zinc-900/90 border-lime-400/50 ring-1 ring-lime-400/20" : item.status === "error" ? "bg-zinc-950/80 border-red-500/30" : "bg-zinc-950/40 border-zinc-800/60 opacity-60"}`}>
                                    <div className="flex items-center gap-3 mb-2 md:mb-0">
                                        <div className="relative flex items-center justify-center w-3 h-3">
                                            ...
                                            {item.status === "processing" && <><span className="absolute w-3 h-3 rounded-full bg-lime-400/40 animate-ping" /><span className="w-2 h-2 rounded-full bg-lime-400" /></>}
