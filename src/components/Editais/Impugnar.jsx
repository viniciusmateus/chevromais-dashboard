...
export default function Impugnar() {
    ...
    // Mapeamento de quais opções necessitam do painel de detalhes
    const objectionsWithDetails = ["delivery", "sample", "ence", "service", "restriction"];
    const hasDetailsToShow = Object.keys(selectedObjections).some(
        (key) => selectedObjections[key] && objectionsWithDetails.includes(key)
    );

    // Formatação e Validação da Data (DD/MM/AAAA)
    ...
    // Geração Individual Simultânea (Em Paralelo)
    const handleGenerate = async () => {
        ...
        if (disputeDate && !dateRegex.test(disputeDate)) {
        ...
        const commonPayload = {
            ...
            disputeDate,
            cityUF,
            buyer: buyer === "OUTRO" ? otherBuyer : buyer,
            details: {
                delivery: { deliveryStipulate, deliveryUnit },
                sample: { sampleObject, sampleClause },
                ence: { enceSpecs, enceGrades },
        ...
        const tasks = activeObjections.map(async (objectionKey) => {
            ...
            try {
                const response = await fetch("/api/gerar-documentos", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ ...commonPayload, objection: objectionKey }),
                });
                ...
                if (result.success) {
                    setItemStatuses((prev) => ({
                        ...prev,
                        [objectionKey]: { status: "success", file: result.file, error: null },
                    }));
                } else {
                    setItemStatuses((prev) => ({
                        ...prev,
                        [objectionKey]: { status: "error", file: null, error: result.error || "Erro ao gerar." },
                    }));
                }
            } catch (err) {
                setItemStatuses((prev) => ({
                    ...prev,
                    [objectionKey]: { status: "error", file: null, error: "Erro de conexão." },
                }));
            }
        });
    ...
    const handleCopy = (type, file) => {
        let text = "";
        const finalBuyer = buyer === "OUTRO" ? otherBuyer : buyer;
    ...
    return (
        <div className="p-6 flex flex-col gap-6 max-w-6xl mx-auto bg-zinc-950 text-zinc-200 min-h-screen">
                ...
                <div className="border-t border-zinc-800/80 pt-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input type="text" className={inputStyle} value={disputeNumber} onChange={(e) => setDisputeNumber(e.target.value.toUpperCase())} />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" className={inputStyle} value={cityUF} onChange={(e) => setCityUF(e.target.value.toUpperCase())} />
                        </div>
                        <div>
                            <label className={labelStyle}>Órgão</label>
                            <select value={buyer} onChange={(e) => setBuyer(e.target.value)} className={`${inputStyle} h-[42px] cursor-pointer`}>
                                <option value="" disabled className="bg-zinc-900 text-zinc-500">-- Selecione --</option>
                                <option value="PREFEITURA MUNICIPAL DE" className="bg-zinc-900 text-zinc-200">PREFEITURA MUNICIPAL</option>
                                <option value="SEM LOCALIDADE" className="bg-zinc-900 text-zinc-200">SEM LOCALIDADE</option>
                                <option value="COM LOCALIDADE" className="bg-zinc-900 text-zinc-200">COM LOCALIDADE</option>
                                <option value="OUTRO" className="bg-zinc-900 text-zinc-200">OUTRO / PERSONALIZADO</option>
                            </select>
                        </div>
                    </div>
                    {buyer === "OUTRO" && (
                        <div className="mt-4">
                            <label className={labelStyle}>Outro Órgão</label>
                            <input type="text" className={inputStyle} value={otherBuyer} onChange={(e) => setOtherBuyer(e.target.value.toUpperCase())} />
                        </div>
                    )}
                </div>
            </div>
            ...
            {/* DETALHES - Exibição condicional mantida */}
            {hasDetailsToShow && (
                <div className="bg-zinc-900/60 p-5 rounded-xl border border-zinc-800 shadow-xl flex flex-col gap-6">
                        ...
                        <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-lg flex flex-col gap-3">
                            ...
                            <div className="flex flex-col md:flex-row items-end gap-4">
                                <div className="flex-1 w-full">
                                    ...
                                    <input type="text" className={inputStyle} value={deliveryStipulate} onChange={(e) => setDeliveryStipulate(e.target.value.toUpperCase())} />
                                </div>
                        ...
                        <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-lg flex flex-col gap-3">
                            ...
                            <div className="flex flex-col gap-3">
                                <div>
                                    ...
                                    <input type="text" className={inputStyle} value={sampleObject} onChange={(e) => setSampleObject(e.target.value.toUpperCase())} />
                                </div>
                                <div>
                                    <label className={labelStyle}>Cláusulas da Amostra</label>
                                    <textarea rows={3} value={sampleClause} onChange={(e) => setSampleClause(e.target.value)} className={inputStyle} placeholder="Insira as cláusulas de amostra..." />
                                </div>
                            </div>
                        </div>
                    )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        {[{ key: "traction", label: "Aderência" }, { key: "resistance", label: "Resistência" }].map(({ key, label }) => (
                        ...
                        <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-lg flex flex-col gap-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="text" className={inputStyle} value={serviceType} onChange={(e) => setServiceType(e.target.value.toUpperCase())} />
                                </div>
                                <div>
                                    <label className={labelStyle}>Objeto(s) do Serviço</label>
                                    <input type="text" className={inputStyle} value={serviceObject} onChange={(e) => setServiceObject(e.target.value.toUpperCase())} />
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {selectedObjections.restriction && (
                        <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-lg flex flex-col gap-3">
                            ...
                            <div>
                                ...
                                <textarea rows={3} value={restrictionClause} onChange={(e) => setRestrictionClause(e.target.value)} className={inputStyle} placeholder="Insira as cláusulas relativas à restrição..." />
                            </div>
                        </div>
                    )}
                </div>
            ...
            <div className="flex justify-end bg-zinc-900/60 p-4 rounded-xl border border-zinc-800 shadow-xl">
                <button
                    ...
                    className="bg-lime-400 hover:bg-lime-300 text-zinc-950 font-extrabold py-3 px-8 rounded-lg transition-all text-sm shadow-lg shadow-lime-400/20 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                    {isProcessing ? "Gerando Documentos..." : "Gerar Documento(s)"}
                </button>
            </div>

            {/* PAINEL DE PROGRESSO MODERNO E LINHAS DE STATUS */}
            {totalItems > 0 && (
                <div className="bg-zinc-900/60 p-5 rounded-xl border border-zinc-800 shadow-2xl flex flex-col gap-5 backdrop-blur-sm">
                    ...
                    <div className="flex flex-col gap-2.5">
                        <div className="flex justify-between items-end">
                            <div>
                                <span className="text-[10px] font-extrabold uppercase tracking-wider text-lime-400/80 block">
                        ...
                        <div className="w-full h-2.5 bg-zinc-950 rounded-full p-0.5 border border-zinc-800/80 relative overflow-hidden">
                            <div
                                className="h-full bg-gradient-to-r from-lime-500 to-lime-300 rounded-full transition-all duration-300 ease-out shadow-[0_0_12px_rgba(163,230,53,0.4)]"
                                style={{ width: `${progressPercentage}%` }}
                            />
                        </div>
                    </div>

                    {/* Lista Dinâmica de Impugnações */}
                    <div className="flex flex-col gap-2.5">
                            ...
                            return (
                                <div
                                    ...
                                    className={`flex flex-col md:flex-row md:items-center justify-between p-3.5 rounded-lg border transition-all ${
                                        item.status === "success"
                                            ? "bg-zinc-950/80 border-lime-400/30 shadow-sm shadow-lime-400/5"
                                            : item.status === "processing"
                                            ? "bg-zinc-900/90 border-lime-400/50 ring-1 ring-lime-400/20"
                                            : item.status === "error"
                                            ? "bg-zinc-950/80 border-red-500/30"
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-3 mb-2 md:mb-0">
                                        ...
                                        <span className="font-bold text-zinc-200 text-xs md:text-sm">
                                            {label}
                                        </span>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        {item.status === "success" && item.file && (
                                            <div className="flex flex-wrap gap-2">
                                                <a
                                                    href={`data:application/pdf;base64,${item.file.pdfBase64}`}
                                                    download={item.file.pdfFilename}
                                                    className="py-1.5 px-3 rounded-lg font-bold text-xs bg-lime-400 text-zinc-950 hover:bg-lime-300 transition-all shadow-sm shadow-lime-400/20"
                                                >
                                                    Download PDF
                                                </a>

                                                <button
                                                    onClick={() => handleCopy("assunto", item.file)}
                                                    className="py-1.5 px-3 rounded-lg font-bold text-xs bg-zinc-900 border border-zinc-800 text-zinc-300 hover:text-zinc-100 hover:border-zinc-700 transition-all"
                                                >
                                                    {copiedStates[`${item.file.objectionKey}-assunto`] ? "Copiado!" : "Copiar Assunto"}
                                                </button>

                                                <button
                                                    onClick={() => handleCopy("mensagem", item.file)}
                                                    className="py-1.5 px-3 rounded-lg font-bold text-xs bg-zinc-900 border border-zinc-800 text-zinc-300 hover:text-zinc-100 hover:border-zinc-700 transition-all"
                                                >
