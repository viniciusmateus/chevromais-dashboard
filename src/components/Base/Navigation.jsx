		...
		<div className="fixed h-screen bg-zinc-900 text-zinc-200 flex flex-col shadow-lg z-1 border-r border-zinc-800">
			...
			<div className="p-2 bg-zinc-900 border-b border-zinc-800">
				<div className="flex items-center space-x-2">
					<span className="flex justify-center items-center text-base rounded-full bg-zinc-950 border border-zinc-800 text-lime-400 font-bold w-10 h-10 flex-shrink-0 shadow-sm">CH</span>
				</div>
			</div>
			...
			<div className="flex items-center flex-col">
				{mainNavItems.map((item, index) => {
					...
					if (item.type === "link") {
						return (
							<div key={index} className={`group relative ${borderClasses}`}>
								<a href={item.disabled ? undefined : item.href} className={`${item.disabled ? buttomLinkDisabled : buttomLink} my-2`} title={item.title}>
					...
					return (
						<div key={index} className={`group relative ${borderClasses}`}>
							<div className={`${item.disabled ? buttomLinkDisabled : buttomLink} my-2 group-hover:bg-zinc-800/60 group-hover:text-lime-400 cursor-pointer`}>
								<IconComponent sx={iconStyle} />
							</div>
