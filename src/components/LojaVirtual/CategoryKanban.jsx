...
export default function CategoryKanban() {
  ...
  const saveJsonToLocal = async () => {
    ...
    try {
      ...
      if (result.success) {
        alert(
          "Sucesso: Arquivo tire-selection.json salvo na pasta do servidor Node!",
        );
      } else {
        alert(
          "Erro ao salvar arquivo: " + (result.message || "Erro desconhecido"),
        );
      }
    } catch (err) {
      console.error(err);
      alert(
        "Erro de conexão: Verifique se o servidor Node está rodando na porta 3001.",
      );
    } finally {
  ...
  const currentWidths =
    currentCategory &&
    typeof currentCategory.widths === "object" &&
    currentCategory.widths !== null
      ? currentCategory.widths
      : {};

  const currentProfiles =
    selectedWidthKey &&
    typeof currentWidths[selectedWidthKey] === "object" &&
    currentWidths[selectedWidthKey] !== null
  ...
  const sortMapList = (mapType, keysArray) => {
    ...
    return [...keysArray].sort((aKey, bKey) => {
      ...
      const aEntry = Object.values(mapObj).find(
        (item) => String(item?.label) === aLabel,
      );
      const bEntry = Object.values(mapObj).find(
        (item) => String(item?.label) === bLabel,
      );
  ...
  const handleDropMaps = async (e, dropIndex, mapType, itemsList) => {
    ...
    newList.forEach((itemKey, index) => {
      ...
      const foundEntry = mapEntries.find(
        ([_, item]) => String(item?.label) === label,
      );
      ...
      if (e.key === "Enter") {
        ...
        if (!selectedCategoryKey) {
          ...
          openModal(
            "width",
            "add",
            `tire-selection/categories/${selectedCategoryKey}/widths`,
          );
        } else if (!selectedProfileKey) {
          openModal(
            "profile",
            "add",
            `tire-selection/categories/${selectedCategoryKey}/widths/${selectedWidthKey}`,
          );
        } else {
          openModal(
            "rim",
            "add",
            `tire-selection/categories/${selectedCategoryKey}/widths/${selectedWidthKey}/${selectedProfileKey}`,
          );
        }
      }
    };
  ...
  const handleSave = async (e) => {
    ...
    const { type, mode, targetPath, initialValue, rimIndex, mapType, uid } =
      modalConfig;
    ...
    try {
      ...
      if (type === "category") {
        ...
        if (mode === "add") {
          ...
          if (initialValue !== encodedValue) {
            ...
            const mergedRims = Array.from(
              new Set([...existingNewData, ...oldData]),
            );
        ...
        updates[`tire-selection/maps/${mapType}/${uid}/catID`] =
          mapCatIdSelect || "";
        ...
        if (oldLabel && oldLabel !== rawCleanValue) {
          Object.entries(categories).forEach(([catUid, catData]) => {
            ...
            if (mapType === "width" && catData.widths[oldEncodedLabel]) {
              ...
              updates[
                `tire-selection/categories/${catUid}/widths/${oldEncodedLabel}`
              ] = null;
              updates[
                `tire-selection/categories/${catUid}/widths/${encodedValue}`
              ] = widthContent;
              ...
              Object.entries(catData.widths).forEach(([wKey, pObj]) => {
                if (pObj && typeof pObj === "object" && pObj[oldEncodedLabel]) {
                  ...
                  updates[
                    `tire-selection/categories/${catUid}/widths/${wKey}/${oldEncodedLabel}`
                  ] = null;
                  updates[
                    `tire-selection/categories/${catUid}/widths/${wKey}/${encodedValue}`
                  ] = pContent;
                }
              });
            } else if (mapType === "rim") {
                ...
                if (pObj && typeof pObj === "object") {
                  Object.entries(pObj).forEach(([pKey, rArr]) => {
                    if (Array.isArray(rArr) && rArr.includes(oldEncodedLabel)) {
                      const newArr = rArr.map((r) =>
                        r === oldEncodedLabel ? encodedValue : r,
                      );
                      updates[
                        `tire-selection/categories/${catUid}/widths/${wKey}/${pKey}`
                      ] = newArr;
                    }
                  });
                }
      ...
      if (type === "profile" && currentWidths[selectedWidthKey]?._placeholder) {
        updates[
          `tire-selection/categories/${selectedCategoryKey}/widths/${selectedWidthKey}/_placeholder`
        ] = null;
      }
        ...
        if (labelToDelete) {
          Object.entries(categories).forEach(([catUid, catData]) => {
            ...
            if (mapType === "width" && catData.widths[encodedToDelete]) {
              updates[
                `tire-selection/categories/${catUid}/widths/${encodedToDelete}`
              ] = null;
            } else if (mapType === "profile") {
              Object.entries(catData.widths).forEach(([wKey, pObj]) => {
                if (pObj && typeof pObj === "object" && pObj[encodedToDelete]) {
                  updates[
                    `tire-selection/categories/${catUid}/widths/${wKey}/${encodedToDelete}`
                  ] = null;
                }
              });
            } else if (mapType === "rim") {
  ...
  if (error) {
    return (
      <div className="p-4 m-4 bg-red-950 border border-red-800 text-red-200 rounded-2xl font-medium text-sm">
        Erro: {error}
      </div>
    );
  }
  ...
  const rawProfileKeys = sortMapList(
    "profile",
    Object.keys(currentProfiles).filter(
      (k) => k !== "_placeholder" && k !== "-",
    ),
  );
  const orderedProfileKeys = sortSelectedToTop(
    rawProfileKeys,
    selectedProfileKey,
  );
      ...
      <div className="flex items-center justify-between gap-4 p-4 bg-zinc-900 rounded-2xl border border-zinc-800/80 shadow-2xl">
        <div className="flex items-center gap-1.5 p-1.5 bg-zinc-950 rounded-xl w-fit border border-zinc-800/80">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-semibold text-xs tracking-wide uppercase transition-all duration-150 ${activeTab === "kanban" ? "bg-zinc-800 text-slate-50 shadow-md border border-zinc-700" : "text-slate-400 hover:text-slate-50 hover:bg-zinc-800/50"}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-semibold text-xs tracking-wide uppercase transition-all duration-150 ${activeTab === "maps" ? "bg-zinc-800 text-slate-50 shadow-md border border-zinc-700" : "text-slate-400 hover:text-slate-50 hover:bg-zinc-800/50"}`}
          >
        ...
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-xs tracking-wide uppercase bg-lime-400 hover:bg-lime-500 disabled:opacity-50 disabled:cursor-not-allowed text-black shadow-lg shadow-lime-400/10 transition-all duration-150 active:scale-95 border border-lime-500"
          ...
          {[
            {
              title: "Categoria",
              type: "category",
              path: "tire-selection/categories",
              data: orderedCategoryKeys,
              selected: selectedCategoryKey,
            },
            {
              title: "Largura",
              type: "width",
              path: `tire-selection/categories/${selectedCategoryKey}/widths`,
              data: orderedWidthKeys,
              selected: selectedWidthKey,
              disabled: !selectedCategoryKey,
              emptyMsg: "Selecione uma categoria",
            },
            {
              title: "Perfil",
              type: "profile",
              path: `tire-selection/categories/${selectedCategoryKey}/widths/${selectedWidthKey}`,
              data: orderedProfileKeys,
              selected: selectedProfileKey,
              disabled: !selectedWidthKey,
              emptyMsg: "Selecione uma largura",
              isProfile: true,
            },
            {
              title: "Aro",
              type: "rim",
              path: `tire-selection/categories/${selectedCategoryKey}/widths/${selectedWidthKey}/${selectedProfileKey}`,
              data: rawRimKeys,
              disabled: !selectedProfileKey,
              emptyMsg: "Selecione um perfil",
              isRim: true,
            },
          ].map((col) => (
            <div
              key={col.type}
              className={`bg-zinc-900 border border-zinc-800 rounded-2xl shadow-lg overflow-hidden flex flex-col transition-all duration-200 ${col.disabled ? "opacity-30 pointer-events-none" : "opacity-100"}`}
            >
              <div className="p-4 bg-zinc-800 text-white flex justify-between items-center border-b border-zinc-700/80">
                <span className="font-bold text-xs uppercase tracking-wider text-lime-400">
                  {col.title}
                </span>
                <button
                  onClick={() =>
                    !col.disabled && openModal(col.type, "add", col.path)
                  }
                  ...
                  title={`Adicionar ${col.title} (Atalho: ENTER)`}
                >
                  <AddIcon sx={{ fontSize: 16 }} />
                </button>
              </div>
              <div className="divide-y divide-zinc-800/50 max-h-137.5 overflow-y-auto p-1.5 bg-zinc-950 custom-scrollbar">
                {col.disabled ? (
                  <div className="p-6 text-center flex flex-col items-center justify-center border-2 border-dashed border-zinc-800/50 rounded-xl m-2 opacity-70 min-h-25">
                  ...
                  <>
                    ...
                    {col.data.map((key, index) => {
                      const itemData =
                        col.type === "category" ? categories[key] : null;
                      const label =
                        col.type === "category"
                          ? itemData?.label
                          : col.isRim
                      ...
                      return (
                        <div
                          key={uniqueKey}
                          onClick={col.isRim ? undefined : handleColClick}
                        ...
                        >
                          <span
                            className={`truncate text-sm ${col.isRim ? "font-semibold" : ""}`}
                          >
                            {label}
                          </span>
                          <div className="flex gap-1 items-center shrink-0 ml-3">
                            <button
                              ...
                              className={`w-7 h-7 rounded-lg inline-flex items-center justify-center transition-colors ${isSelected ? "bg-black/10 hover:bg-black/20 text-black" : "bg-zinc-800 hover:bg-zinc-700 text-slate-400 hover:text-slate-100 border border-zinc-700/80"}`}
                            >
                            ...
                            <button
                              ...
                              className={`w-7 h-7 rounded-lg inline-flex items-center justify-center transition-colors ${isSelected ? "bg-black/20 hover:bg-black text-black hover:text-lime-400 border border-transparent" : "bg-black hover:bg-zinc-950 text-zinc-400 hover:text-white border border-zinc-800/80"}`}
                            >
      ...
      {activeTab === "maps" && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl shadow-lg overflow-hidden flex flex-col">
            <div className="p-4 bg-zinc-800 text-white font-bold text-xs uppercase tracking-wider flex justify-between items-center border-b border-zinc-700/80">
              ...
              <span className="text-[10px] bg-zinc-700 text-slate-200 px-2.5 py-1 rounded-md font-medium border border-zinc-600">
                ...
                {Object.keys(categories).length === 1 ? "item" : "itens"}
              </span>
            </div>
            <div className="divide-y divide-zinc-800/50 max-h-137.5 overflow-y-auto p-1.5 bg-zinc-950 custom-scrollbar">
              {Object.keys(categories).length === 0 ? (
                ...
                Object.entries(categories).map(([catUid, catData]) => {
                  ...
                  return (
                    <div
                      key={catUid}
                      className="p-3.5 my-1 rounded-xl flex justify-between items-center bg-zinc-900 hover:bg-zinc-800/80 border border-zinc-800/80 hover:border-zinc-700 transition-all group"
                    >
                      <div className="flex items-center gap-3">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-base font-bold text-slate-50">
                            {catData?.label || catUid}
                          </span>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            {/* PILL VERDE PARA PREENCHIDO, VERMELHO PARA VAZIO */}
                            <span
                              className={`text-[10px] font-mono px-2 py-0.5 rounded-md border ${hasUrl ? "bg-lime-950/80 text-lime-400 border-lime-500/40 font-semibold" : "bg-red-950/80 text-red-400 border-red-500/40 font-semibold"}`}
                            >
                        ...
                        <button
                          ...
                          onClick={() =>
                            openModal(
                              "category",
                              "edit",
                              `tire-selection/categories/${catUid}`,
                              catData?.label,
                              null,
                              "",
                              catUid,
                          ...
                          className="w-8 h-8 rounded-lg inline-flex items-center justify-center bg-zinc-800 hover:bg-zinc-700 text-slate-400 hover:text-slate-100 border border-zinc-700/80 transition-colors"
                        >
                          <EditIcon sx={{ fontSize: 16 }} />
                        </button>
                        <button
                          ...
                          onClick={() =>
                            handleDelete(
                              "tire-selection/categories",
                              catUid,
                              "category",
                            )
                          }
                          className="w-8 h-8 rounded-lg inline-flex items-center justify-center bg-black hover:bg-zinc-950 text-zinc-400 hover:text-white border border-zinc-800/80 transition-colors"
                        >
                          <DeleteIcon sx={{ fontSize: 16 }} />
                        </button>
                      </div>
          ...
          {["width", "profile", "rim"].map((mapType) => {
            ...
            const keysList = sortedMapEntries.map(([_, item]) =>
              encodeKey(item?.label),
            );

            return (
              <div
                key={mapType}
                className="bg-zinc-900 border border-zinc-800 rounded-2xl shadow-lg overflow-hidden flex flex-col"
              >
                <div className="p-4 bg-zinc-800 text-white font-bold text-xs uppercase tracking-wider flex justify-between items-center border-b border-zinc-700/80">
                  <span className="text-lime-400">Maps: {mapType}</span>
                  <span className="text-[10px] bg-zinc-700 text-slate-200 px-2.5 py-1 rounded-md font-medium border border-zinc-600">
                    {sortedMapEntries.length}{" "}
                    {sortedMapEntries.length === 1 ? "item" : "itens"}
                  </span>
                </div>
                <div className="divide-y divide-zinc-800/50 max-h-137.5 overflow-y-auto p-1.5 bg-zinc-950 custom-scrollbar">
                  {sortedMapEntries.length === 0 ? (
                    ...
                    sortedMapEntries.map(([uid, item], index) => {
                      const usageCats = getUsageCategories(
                        item?.label,
                        mapType,
                      );
                      ...
                      return (
                        <div
                          ...
                          onDrop={(e) =>
                            handleDropMaps(e, index, mapType, keysList)
                          }
                          className="p-3.5 my-1 rounded-xl flex justify-between items-center bg-zinc-900 hover:bg-zinc-800/80 border border-zinc-800/80 hover:border-zinc-700 transition-all cursor-grab active:cursor-grabbing group"
                        >
                          <div className="flex items-center gap-3">
                            <DragHandleIcon
                              sx={{ fontSize: 16 }}
                            ...
                            <div className="flex flex-col gap-0.5">
                              ...
                              <div className="flex items-center gap-1.5 mt-0.5">
                                {/* PILL VERDE PARA PREENCHIDO, VERMELHO PARA VAZIO */}
                                <span
                                  className={`text-[10px] font-mono px-2 py-0.5 rounded-md border ${hasCatId ? "bg-lime-950/80 text-lime-400 border-lime-500/40 font-semibold" : "bg-red-950/80 text-red-400 border-red-500/40 font-semibold"}`}
                                >
                          ...
                          <div className="flex gap-1 items-center shrink-0 ml-3">
                            <button
                              title="Editar"
                              onClick={() =>
                                openEditMapModal(mapType, uid, item)
                              }
                              className="w-8 h-8 rounded-lg inline-flex items-center justify-center bg-zinc-800 hover:bg-zinc-700 text-slate-400 hover:text-slate-100 border border-zinc-700/80 transition-colors"
                            >
                              <EditIcon sx={{ fontSize: 16 }} />
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleDelete(
                                  `tire-selection/maps/${mapType}/${uid}`,
                                  null,
                                  "mapItem",
                                )
                              }
          ...
          <div className="bg-zinc-900 rounded-2xl p-6 w-full max-w-md shadow-2xl border border-zinc-800 transition-all scale-100">
            ...
            <form onSubmit={handleSave}>
              <div className="flex flex-col gap-5 mb-7">
                {[
                  ...
                  },
                  {
                    label: "URL do Site:",
                    value: mapUrlInput,
                    setter: setMapUrlInput,
                    placeholder: "Ex: /maquinas-agricolas",
                    show: modalConfig.type === "category",
                  },
                  {
                    label: "catID (ID do Site):",
                    value: mapCatIdSelect,
                    setter: setMapCatIdSelect,
                    placeholder: "Ex: 3152185",
                    show: modalConfig.type === "mapItem",
                  },
                ].map(
                  (field, idx) =>
                    field.show !== false && (
                      <div key={idx}>
                        ...
                        <input
                          ...
                          className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-lime-400 font-medium text-slate-100 text-sm transition-all placeholder:text-zinc-600"
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-zinc-800">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-lime-400 text-black rounded-xl hover:bg-lime-500 active:scale-95 font-bold text-xs tracking-wide transition-all shadow-lg border border-lime-500"
                >
                  Salvar
                </button>
              </div>
