...
export const POST = async () => {
  ...
  if (!host || !user || !pass) {
    return new Response(
      JSON.stringify({ success: false, error: 'Credenciais IMAP não configuradas no .env' }),
  ...
  try {
    ...
    for await (let message of client.fetch('1:*', { source: true, envelope: true })) {
      ...
      messages.push({
        message_id: message.envelope.messageId || `msg_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`,
        sender_name: parsed.from?.value[0]?.name || '',
        sender_email: parsed.from?.value[0]?.address || 'desconhecido@email.com',
        recipient_email: parsed.to?.value[0]?.address || '',
        subject: parsed.subject || '(Sem assunto)',
        body_text: parsed.text || '',
        body_html: parsed.html || parsed.textAsHtml || `<p>${parsed.text || ''}</p>`,
        is_read: false,
