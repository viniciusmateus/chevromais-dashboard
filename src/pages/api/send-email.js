...
export async function POST({ request }) {
  ...
  try {
    ...
    const host = await getAppSetting(supabase, "SMTP_HOST");
    const port = Number((await getAppSetting(supabase, "SMTP_PORT")) || 465);
    const user = await getAppSetting(supabase, "SMTP_USER");
    const pass = await getAppSetting(supabase, "SMTP_PASS");

    if (!user || !pass) {
      return new Response(
        JSON.stringify({
          error: "Credenciais SMTP ausentes no banco de dados ou .env",
        }),
        { status: 500 },
      );
    }
